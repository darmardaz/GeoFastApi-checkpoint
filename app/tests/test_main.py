...
def test_get_voivodeship(test_app, monkeypatch):
    ...
    response = test_app.post("/api/check-voiv/", data=json.dumps(test_request_payload))
...
def test_get_event(test_app, monkeypatch, event):
    ...
    response = test_app.get("/api/event/1")
...
def test_create_event(test_app, monkeypatch):
    ...
    response = test_app.post("/api/event/", json.dumps(test_request_payload))
